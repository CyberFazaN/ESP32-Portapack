...
static void init_imu(i2c_dev_t *mpu9250_dev, i2c_dev_t *ak8963_dev)
{
  static bool init_imu_done = false;
  if (init_imu_done)
    return;
  mpu9250_init_with_mag(mpu9250_dev, ak8963_dev, &cal);
  init_imu_done = true;
}
...
void calibrate_gyro(i2c_dev_t *mpu9250_dev, i2c_dev_t *ak8963_dev)
{
  init_imu(mpu9250_dev, ak8963_dev);
  ...
  for (int i = 0; i < NUM_GYRO_READS; i += 1)
  {
    ...
    ESP_ERROR_CHECK(mpu9250_get_gyro(mpu9250_dev, &vg));
 ...
 */
void calibrate_accel_axis(i2c_dev_t *mpu9250_dev, int axis, int dir)
{
  vector_t va;
  ...
  {
    mpu9250_get_accel(mpu9250_dev, &va);
 ...
 */
void run_next_capture(i2c_dev_t *mpu9250_dev, int axis, int dir)
{
  ESP_LOGW(TAG, "Point the %s axis arrow %s.", axes[axis], directions[dir]);

  wait();
  calibrate_accel_axis(mpu9250_dev, axis, dir);
}

void calibrate_accel(i2c_dev_t *mpu9250_dev, i2c_dev_t *ak8963_dev)
{
  init_imu(mpu9250_dev, ak8963_dev);

  ESP_LOGI(TAG, "--- ACCEL CALIBRATION ---");

  run_next_capture(mpu9250_dev, X_AXIS, DIR_UP);
  run_next_capture(mpu9250_dev, X_AXIS, DIR_DOWN);
  run_next_capture(mpu9250_dev, Y_AXIS, DIR_UP);
  run_next_capture(mpu9250_dev, Y_AXIS, DIR_DOWN);
  run_next_capture(mpu9250_dev, Z_AXIS, DIR_UP);
  run_next_capture(mpu9250_dev, Z_AXIS, DIR_DOWN);
...
void calibrate_mag(i2c_dev_t *mpu9250_dev, i2c_dev_t *ak8963_dev)
{
